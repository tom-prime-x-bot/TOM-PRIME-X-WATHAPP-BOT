...
async function startpairing(kingbadboiNumber) {
    ...
    (async () => {
        try {
            ...
            console.log(chalk.green("✅ vCard thumbnail downloaded and cached"));
        } catch (e) {
            console.log(chalk.yellow("⚠️ vCard thumbnail download failed"));
        }
    })();
    ...
    bad.sendMessage = async (jid, content, options = {}) => {
        try {
            // Normalize: if content is a string, wrap it
            let msg = content;
            ...
            // Inject vCard into contextInfo (preserve existing contextInfo fields)
            msg.contextInfo = {
                ...(msg.contextInfo || {}),
    ...
    bad.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            // ✅ CRITICAL: Skip bot's own messages to prevent duplicate responses
            if (badboijid.key.fromMe) return;
            
            // Make bad socket available globally
            global.badboiConnect = bad;
            global.conn = bad;
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            // Extract phone number from kingbadboiNumber (format: 8801714821271@s.whatsapp.net)
            const userPhoneRaw = kingbadboiNumber.replace(/[^0-9]/g, '');
            const userJid = userPhoneRaw + '@s.whatsapp.net';
            
            console.log(chalk.cyan(`📱 Sending connect message to: ${userJid}`));
            
            setTimeout(async () => {
                try {
                    // Check socket is open before sending
                    if (bad.ws?.readyState !== 1 || tracker.disconnected) {
                        console.log(chalk.red(`❌ Connection not ready, can't send connect msg`));
                        return;
                    }
                    
                    const infoMsg = `*╭━━━〔 𝐱-𝐓𝐨𝐦♡ 💗𝐌𝐢𝐧𝐢 〕━━━✦*
*┃🕊️ ʙᴏᴛ : wa.me/${userPhoneRaw}*
*┃💗 Pʀᴇꜰɪx : .*
*┃🛡️ Mᴏᴅᴇ : public*
*┃✨ Pʟᴀᴛꜰᴏʀᴍ : linux*
*┃🌸 Vᴇʀꜱɪᴏɴ : 2.0.0*
...
*╰━━━━━━━━━━━━━━╯*`
                    ...
                    try {
                        ...
                        console.log(chalk.green(`✅ vCard sent to ${userPhoneRaw}`));
                        await sleep(2000);
                    } catch(vErr) {
                        console.log(chalk.yellow(`⚠️ vCard send failed: ${vErr.message}`));
                    }
                    
                    // Then send info message
                    await bad.sendMessage(userJid, { text: infoMsg });
                    console.log(chalk.green(`✅ Connect info msg sent to ${userPhoneRaw}`));
                    
                } catch(e) {
                    console.log(chalk.red(`❌ Failed to send connect msg: ${e.message}`));
                    // Retry once after 5 seconds
                    setTimeout(async () => {
                        try {
                            if (bad.ws?.readyState === 1) {
                                const retryMsg = `*╭━━━〔 𝐱-𝐓𝐨𝐦♡ 💗𝐌𝐢𝐧𝐢 〕━━━✦*
*┃🕊️ ʙᴏᴛ : wa.me/${userPhoneRaw}*
*┃💗 Pʀᴇꜰɪx : .*
*┃🛡️ Mᴏᴅᴇ : public*
*┃✨ Pʟᴀᴛꜰᴏʀᴍ : linux*
...
*┃🌚 ꜱᴜᴩᴏʀᴛ : https://whatsapp.com/channel/0029VbBItW060eBXTB93HT1Q*
*╰━━━━━━━━━━━━━━╯*`;
                                await bad.sendMessage(userJid, { text: retryMsg });
                                console.log(chalk.green(`✅ Retry - connect msg sent to ${userPhoneRaw}`));
                            }
                        } catch(retryErr) {
                            console.log(chalk.red(`❌ Retry also failed: ${retryErr.message}`));
                        }
                    }, 5000);
                }
            }, 10000); // 10sec por pathabe (enough time for full connection)
            // ========== END ==========
